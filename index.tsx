...
const DashboardView = ({ userData, getDayProgress, hasMedalToday, setCurrentView, setSelectedArea }: any) => (
    ...
    return (
      <div>
        ...
        <main className="view-container" style={{maxWidth: '56rem'}}>
            <div className="tabs">
                <div className="tabs-list" role="tablist" aria-label="Opções da área de saúde">
                    <button id="tab-checklist" className="tab-trigger" role="tab" aria-selected={activeTab === 'checklist'} aria-controls="panel-checklist" onClick={() => setActiveTab('checklist')}>Checklist Diário</button>
                    <button id="tab-goals" className="tab-trigger" role="tab" aria-selected={activeTab === 'goals'} aria-controls="panel-goals" onClick={() => setActiveTab('goals')}>Metas & Reflexões</button>
                    <button id="tab-notes" className="tab-trigger" role="tab" aria-selected={activeTab === 'notes'} aria-controls="panel-notes" onClick={() => setActiveTab('notes')}>Notas Rápidas</button>
                </div>
                {activeTab === 'checklist' && <div id="panel-checklist" className="tab-content" role="tabpanel" aria-labelledby="tab-checklist">
                    <div className="card"><div className="card-content">
                        ...
                        <div className="space-y-3">
                            {area.bookTasks.map((task, index) => {
                                ...
                                const handleToggle = () => toggleTask(selectedArea, 'book', index);
                                return (<div key={taskId} className={`task-item ${isCompleted ? 'completed': ''}`}>
                                    <div className="task-item-toggle" role="button" tabIndex={0} onClick={handleToggle} onKeyDown={(e) => handleKeyboardClick(e, handleToggle)}>
                                        {isCompleted ? <CheckCircle2Icon className="flex-shrink-0" /> : <CircleIcon className="flex-shrink-0" />}
                                        <span className="task-text">{task}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        {currentAlarm ? <button className="btn btn-ghost icon-btn" onClick={() => clearAlarm(selectedArea, 'book', index)} aria-label={`Cancelar alarme das ${currentAlarm.time}`}><BellOffIcon /></button> : <input type="time" className="input" aria-label={`Definir alarme para a tarefa ${task}`} value={alarmTime[taskId] || ''} onChange={e => setAlarmTime(p => ({...p, [taskId]: e.target.value}))} />}
                                        <button className="btn btn-ghost icon-btn" onClick={() => setAlarm(selectedArea, 'book', index, alarmTime[taskId])} aria-label="Confirmar alarme"><BellIcon /></button>
                                    </div>
                                </div>);
                            })}
                        </div>
                        ...
                        <h3 className="flex items-center gap-2" style={{fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem'}}>
                         ...
                         <div className="space-y-3">
                            <div className="flex gap-2">
                                ...
                                <button className="btn btn-green" onClick={() => {addCustomTask(selectedArea, newTaskText); setNewTaskText('');}} aria-label="Adicionar nova tarefa"><PlusIcon /></button>
                            </div>
                            {(userData.customTasks[selectedArea] || []).map((task: string, index: number) => {
                                ...
                                const currentAlarm = userData.alarms[taskId];
                                const handleToggle = () => toggleTask(selectedArea, 'custom', index);
                                return (<div key={taskId} className={`task-item ${isCompleted ? 'completed': ''}`}>
                                     <div className="task-item-toggle" role="button" tabIndex={0} onClick={handleToggle} onKeyDown={(e) => handleKeyboardClick(e, handleToggle)}>
                                        {isCompleted ? <CheckCircle2Icon className="flex-shrink-0" /> : <CircleIcon className="flex-shrink-0" />}
                                        <span className="task-text">{task}</span>
                                    ...
                                    <div className="flex items-center gap-2">
                                        {currentAlarm ? <button className="btn btn-ghost icon-btn" onClick={() => clearAlarm(selectedArea, 'custom', index)} aria-label={`Cancelar alarme das ${currentAlarm.time}`}><BellOffIcon /></button> : <input type="time" aria-label={`Definir alarme para a tarefa ${task}`} className="input" value={alarmTime[taskId] || ''} onChange={e => setAlarmTime(p => ({...p, [taskId]: e.target.value}))} />}
                                        <button className="btn btn-ghost icon-btn" onClick={() => setAlarm(selectedArea, 'custom', index, alarmTime[taskId])} aria-label="Confirmar alarme"><BellIcon /></button>
                                    </div>
                                    <button className="btn btn-ghost icon-btn" onClick={() => removeCustomTask(selectedArea, index)} aria-label={`Remover tarefa: ${task}`}><XIcon /></button>
                                </div>);
                            })}
                        </div>
                    </div></div>
                ...
                {activeTab === 'goals' && <div id="panel-goals" className="tab-content" role="tabpanel" aria-labelledby="tab-goals">
                    <div className="card"><div className="card-content">
                        ...
                        <textarea placeholder="Defina sua meta SMART para esta semana..." value={weeklyGoal} onChange={e => {setWeeklyGoal(e.target.value); setUserData((p: any) => ({...p, weeklyGoals: {...p.weeklyGoals, [selectedArea]: e.target.value}}));}} className="textarea" style={{minHeight: '100px'}} />
                    ...
                    <div className="card"><div className="card-content">
                        ...
                        <div className="flex gap-2">
                           <textarea placeholder="Registre suas conquistas e reflexões do dia..." value={todayAchievement} onChange={e => {setTodayAchievement(e.target.value); setUserData((p: any) => ({...p, achievements: {...p.achievements, [selectedArea]: e.target.value}}));}} className="textarea flex-1" />
                           <button className="btn btn-ghost icon-btn" style={{border: '1px solid var(--border-color)'}} aria-label="Gravar áudio"><MicIcon /></button>
                        </div>
                    </div></div>
                </div>}
                {activeTab === 'notes' && <div id="panel-notes" className="tab-content" role="tabpanel" aria-labelledby="tab-notes">
                     <div className="card"><div className="card-content">
                        ...
                        <textarea placeholder="Anote insights, lembretes ou reflexões..." value={quickNotes} onChange={e => {setQuickNotes(e.target.value); setUserData((p: any) => ({...p, notes: {...p.notes, [selectedArea]: e.target.value}}));}} className="textarea" style={{minHeight: '200px'}} />
                    </div></div>
...
const RewardsView = ({ userData, getDayProgress, setCurrentView }: any) => {
    const totalMedals = Object.values(userData.medals || {}).reduce((total: number, areaMedals: any) => total + Object.keys(areaMedals || {}).length, 0);
    // FIX: Cast Object.values result to any[] to use spread operator, as it returns unknown[] for un-typed objects.
    const maxStreak: number = Math.max(...(Object.values(userData.streaks || {}) as any[]).map((s: any) => Number(s) || 0), 0);
...
function App() {
  ...
  const [userData, setUserData] = useState(() => {
    try {
      ...
      // Ensure all keys exist to prevent errors on first load
      return {
        ...
        completedToday: initialData.completedToday || {},
        customTasks: initialData.customTasks || {},
        alarms: initialData.alarms || {}
      }
    } catch (e) {
  ...
  const removeCustomTask = useCallback((areaKey: string, taskIndex: number) => {
    setUserData((prev: any) => {
        ...
        const oldCustomTasks = prev.customTasks[areaKey] || [];
        const newCustomTasks = oldCustomTasks.filter((_: string, i: number) => i !== taskIndex);

        const oldCompletedTodayForArea = prev.completedToday[areaKey]?.[today] || [];
        const newCompletedTodayForArea = oldCompletedTodayForArea
            .map((id: string) => {
                if (!id.startsWith('custom-')) return id;
                const completedIndex = parseInt(id.split('-')[1], 10);
                if (completedIndex < taskIndex) return id;
                if (completedIndex === taskIndex) return null;
                return `custom-${completedIndex - 1}`;
            })
            .filter((id: string | null): id is string => id !== null);

        const newAlarms = { ...prev.alarms };
        delete newAlarms[`${areaKey}-custom-${taskIndex}`];
        for (let i = taskIndex + 1; i < oldCustomTasks.length; i++) {
            const oldTaskId = `${areaKey}-custom-${i}`;
            const newTaskId = `${areaKey}-custom-${i - 1}`;
            if (newAlarms[oldTaskId]) {
                newAlarms[newTaskId] = newAlarms[oldTaskId];
                delete newAlarms[oldTaskId];
            }
        }

        return {
            ...prev,
            customTasks: { ...prev.customTasks, [areaKey]: newCustomTasks },
            completedToday: { ...prev.completedToday, [areaKey]: { ...(prev.completedToday[areaKey] || {}), [today]: newCompletedTodayForArea } },
            alarms: newAlarms,
        };
    });
  }, []);
  ...
  const toggleTask = useCallback((areaKey: string, taskType: string, taskIndex: number) => {
    setUserData((prev: any) => {
      ...
      const newTodayTasks = todayTasks.includes(taskId)
          ? todayTasks.filter((id: string) => id !== taskId)
          : [...todayTasks, taskId];
      
      const newData = {
        ...prev,
        completedToday: { ...prev.completedToday, [areaKey]: { ...areaData, [today]: newTodayTasks } }
      }

      const area = healthAreas[areaKey as keyof typeof healthAreas];
      const totalTasks = area.bookTasks.length + (prev.customTasks[areaKey] || []).length;

      if (newTodayTasks.length === totalTasks && totalTasks > 0 && !prev.medals[areaKey]?.[today]) {
        newData.medals = { ...prev.medals, [areaKey]: { ...prev.medals[areaKey], [today]: true } }
        const streak = calculateStreak(areaKey, newData.medals)
        newData.streaks = { ...prev.streaks, [areaKey]: streak }
      }
      return newData
    });
  }, [calculateStreak]);
  ...
  const setAlarm = useCallback((areaKey: string, taskType: string, taskIndex: number, time: string) => {
    ...
    const scheduleNotification = () => {
        const timeoutId = setTimeout(() => {
            new Notification('Pequenos Passos', { body: `Hora de ${taskText} na área de ${area.name}!`, icon: '/favicon.ico' });
            clearAlarm(areaKey, taskType, taskIndex);
        }, timeToAlarm);
        setUserData((prev: any) => ({ ...prev, alarms: { ...prev.alarms, [taskId]: { time, timeoutId: timeoutId, taskText, areaName: area.name }}}));
  ...
  }, [userData.customTasks, clearAlarm]);
